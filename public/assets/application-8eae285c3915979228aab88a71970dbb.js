...
$(function() {
    $("#initial_reply_on").datepicker();
    $("#applied_on").datepicker();
    $("#sent_challenge_on").datepicker();
    $("#completed_challenge_on").datepicker();
    $("#reviewed_on").datepicker();
    $("#resubmitted_challenge_on").datepicker();
    $("#decision_made_on").datepicker();
...
(function() {
  ...
  Services.Request = (function() {
    ...
    Request._post = function(url, data, callback) {
      return $.ajax({
        type: "POST",
        dataType: 'json',
        url: url,
    ...
    Request._get = function(url, dataType, callback) {
      return $.ajax({
        type: "GET",
        dataType: dataType,
        url: url,
...
(function() {
  ...
  FormHelper.FillDateOnChange = (function() {
    function FillDateOnChange(listenElement) {
      this.listenElement = listenElement;
    }

    FillDateOnChange.prototype.init = function(updateElement) {
      this.updateElement = updateElement;
      return $(this.listenElement).on("change", (function(_this) {
        return function() {
          return _this.addDate(_this.updateElement);
        };
      })(this));
    };

    FillDateOnChange.prototype.addDate = function(updateElement) {
      if (!$(updateElement).val()) {
        return $(updateElement).datepicker('setDate', new Date());
      }
    };

    return FillDateOnChange;

  })();

  FormHelper.DisplayDivOnChange = (function() {
    function DisplayDivOnChange(listenElement) {
      this.listenElement = listenElement;
    }

    DisplayDivOnChange.prototype.init = function(updateElement) {
      this.updateElement = updateElement;
      this.determineShowStatus(this.listenElement, this.updateElement);
      return $(this.listenElement).on("change", (function(_this) {
        return function() {
          return _this.determineShowStatus(_this.listenElement, _this.updateElement);
        };
      })(this));
    };

    DisplayDivOnChange.prototype.determineShowStatus = function(listenElement, updateElement) {
      var value;
      value = $(listenElement).val();
      if (value === "yes") {
        return $(updateElement).slideDown();
      } else {
        return $(updateElement).slideUp();
      }
    };

    return DisplayDivOnChange;

  })();

  $(function() {
    new FormHelper.FillDateOnChange("[data-id=hired-select]").init("#decision_made_on");
    return new FormHelper.DisplayDivOnChange("[data-id=hired-select]").init("#apprenticeship-date-selector");
  });
...
(function() {
  ...
  StateHelper.DatePicker = (function() {
    function DatePicker(listenElement) {
      this.listenElement = listenElement;
    }

    DatePicker.prototype.init = function() {
      return this.bindListenElement();
    };

    DatePicker.prototype.bindListenElement = function() {
      return $(this.listenElement).on("click", (function(_this) {
        return function(event) {
          _this.addDatePicker(event.target);
          return _this.addDatepickerRemover();
        };
      })(this));
    };

    DatePicker.prototype.addDatePicker = function(target) {
      var _that, card, offset, stateElementOffset;
      _that = this;
      card = $(target).closest(".card");
      card.append("<div id='picker'/>");
      $("#picker").datepicker({
        onSelect: function() {
          return _that.postDate(this.value, card);
        }
      });
      stateElementOffset = $(target).position().left;
      offset = _that.getDatePickerPosition(card.width(), stateElementOffset);
      return $(".hasDatepicker").css({
        "z-index": "1000",
        "position": "absolute",
        "margin-top": "10px",
    ...
    DatePicker.prototype.getDatePickerPosition = function(availableWidth, elementOffset) {
      var datePickerWidth;
      datePickerWidth = $(".ui-datepicker").width();
      if ((elementOffset + datePickerWidth) > availableWidth) {
        return availableWidth - datePickerWidth;
      } else {
        return elementOffset;
      }
    };

    DatePicker.prototype.postDate = function(date, card) {
      var data;
      data = {
        applicant: {}
      };
      data.applicant[this.listenElement.slice(1)] = date;
      Services.Request._post("/update_state/" + (card.data("applicantId")), data, (function(_this) {
        return function(response) {
          return _this.render(response, card);
        };
      })(this));
      return this.removeDatePicker();
    };

    DatePicker.prototype.render = function(response, card) {
      if (response.success === "true") {
        $(card).find(".current-state").removeClass("current-state");
        $(card).find(this.listenElement).addClass("current-state");
        return $('#flash-notices').empty();
      } else {
        $('#flash-notices').append("<p class='flash warning'>" + response.message + "</p>");
        return window.location.href = "#flash-notices";
      }
    };

    DatePicker.prototype.addDatepickerRemover = function() {
      ...
      return this.bindDatepickerRemover();
    };

    DatePicker.prototype.bindDatepickerRemover = function() {
      return $("#close-picker").on("click", (function(_this) {
        return function() {
          return _this.removeDatePicker();
        };
      })(this));
    };
  ...
  StateHelper.GenerateOfferLetterForm = (function() {
    function GenerateOfferLetterForm(listenElement) {
      this.listenElement = listenElement;
    }

    GenerateOfferLetterForm.prototype.init = function() {
      return this.bindListenElement();
    };

    GenerateOfferLetterForm.prototype.bindListenElement = function() {
      return $(this.listenElement).on("click", (function(_this) {
        return function(event) {
          var applicantId;
          event.preventDefault();
          applicantId = $(event.target).closest(".card");
          return Services.Request._get("/applicants/" + (applicantId.data("applicantId")) + "/offer_letter_form", 'html', function(response) {
            return _this.openDialog(response, applicantId);
          });
        };
      })(this));
    };

    GenerateOfferLetterForm.prototype.download = function(response) {
      var card;
      if (response.success === "true") {
        ...
        return window.location.replace("/applicants/" + response.applicantId + "/offer_letter.pdf?duration=" + response.duration + "&pt_ft=" + response.pt_ft + "&hours_per_week=" + response.hours_per_week + "&withdraw_offer_date=" + response.withdraw_offer_date);
      } else {
        return $('#modal-errors').text(response.message);
      }
    };

    GenerateOfferLetterForm.prototype.openDialog = function(view, applicantId) {
      $("#offer-letter-form").remove();
      applicantId.append(view);
      $("#offer-letter-form").dialog(this.dialogOptions(applicantId));
      $("[data-id=start_date]").datepicker();
      $("[data-id=withdraw_offer_date]").datepicker();
      this.disableDownloadButton();
      return $("input").change((function(_this) {
        return function() {
    ...
    GenerateOfferLetterForm.prototype.disableDownloadButton = function() {
      $(".ui-button-text").css("color", "#CCCCCC");
      return $("span:contains('Download')").parent().prop("disabled", true);
    };

    GenerateOfferLetterForm.prototype.enableDownloadButton = function() {
      $(".ui-button-text").css("color", "#555555");
      return $("span:contains('Download')").parent().prop("disabled", false);
    };

    GenerateOfferLetterForm.prototype.validateInput = function() {
      var confirmedWithAdmin, validInputs;
      confirmedWithAdmin = $("#confirmed_with_admin").prop('checked');
      validInputs = this.isValidInputs();
    ...
    GenerateOfferLetterForm.prototype.isValidInputs = function() {
      var input_values;
      input_values = [];
      $("#offer-letter-form input").each(function() {
        return input_values.push($(this).val());
      });
      return input_values.every(function(x) {
        return x !== "";
      });
    };

    GenerateOfferLetterForm.prototype.dialogOptions = function(applicantId) {
      return {
        modal: true,
        width: 500,
        ...
        buttons: [
          {
            ...
            click: (function(_this) {
              return function(event) {
                return _this.postEmploymentDates(_this.decisionFormValues(), applicantId);
              };
            })(this)
          }
        ]
    ...
    GenerateOfferLetterForm.prototype.decisionFormValues = function() {
      return {
        applicant: {
          offered_on: new Date(),
    ...
    GenerateOfferLetterForm.prototype.postEmploymentDates = function(data, applicantId) {
      return Services.Request._post("/applicants/" + (applicantId.data("applicantId")) + "/update_employment_dates", data, (function(_this) {
        return function(response) {
          return _this.download(response);
        };
      })(this));
    };

    return GenerateOfferLetterForm;

  })();

  StateHelper.ConfirmHireApplicant = (function() {
    function ConfirmHireApplicant(listenElement) {
      this.listenElement = listenElement;
    }

    ConfirmHireApplicant.prototype.init = function() {
      return this.bindListenElement();
    };

    ConfirmHireApplicant.prototype.bindListenElement = function() {
      return $(this.listenElement).on("click", (function(_this) {
        return function(event) {
          var applicantId;
          event.preventDefault();
          applicantId = $(event.target).closest(".applicant-actions");
          return Services.Request._get("/applicants/" + (applicantId.data("applicantId")) + "/hire", 'html', function(response) {
            return _this.openDialog(response, applicantId);
          });
        };
      })(this));
    ...
    ConfirmHireApplicant.prototype.openDialog = function(view, applicantId) {
      $("#confirm-hire").remove();
      applicantId.append(view);
      return $("#confirm-hire").dialog(this.dialogOptions(applicantId));
    };

    ConfirmHireApplicant.prototype.dialogOptions = function(applicantId) {
      return {
        modal: true,
        width: 500,
        ...
        buttons: [
          {
            ...
            click: (function(_this) {
              return function(event) {
                return window.location.replace("/applicants/" + (applicantId.data("applicantId")) + "/onboarding_letters.pdf");
              };
            })(this)
          }, {
            text: "Submit",
            click: (function(_this) {
              return function(event) {
                return _this.postDecision(_this.confirmHireValues(), applicantId);
              };
            })(this)
          }
        ]
    ...
    ConfirmHireApplicant.prototype.confirmHireValues = function() {
      return {
        applicant: {
    ...
    ConfirmHireApplicant.prototype.postDecision = function(data, applicantId) {
      return $("#confirm-hire form:first-child").submit();
    };

    ConfirmHireApplicant.prototype.render = function(response) {
      if (response.success === "true") {
        return window.location.reload();
      } else {
        $('#flash-notices').append("<p class='flash warning'>" + response.message + "</p>");
        window.location.href = "#flash-notices";
        return $("#confirm-hire").remove();
      }
    };

    return ConfirmHireApplicant;

  })();

  $(function() {
    var i, len, ref, state;
    ref = [".applied_on", ".initial_reply_on", ".sent_challenge_on", ".completed_challenge_on", ".reviewed_on"];
    for (i = 0, len = ref.length; i < len; i++) {
      state = ref[i];
      new StateHelper.DatePicker(state).init();
    }
    new StateHelper.GenerateOfferLetterForm(".offered_on").init();
    return new StateHelper.ConfirmHireApplicant(".decision_made_on").init();
  });
